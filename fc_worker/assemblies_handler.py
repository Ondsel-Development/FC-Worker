...
def download_links_files(doc, model_id, directory_path, headers):
    ...
    link_files = []
    missing_files = find_missing_links(doc)
    ...
    if model_resp.ok:
        ...
        if directory_resp.ok:
            ...
            for file_name in missing_files:
                ...
                link_files.append(file_output_path)
    logger.debug(f"Linked files: {str(link_files)}")
    return link_files
...
def download_assemblies(model_id, input_file, tmp_dir, headers):
    ...
    try:
        ...
        if len(missing_file):
            link_files = download_links_files(doc, model_id, str(tmp_dir), headers)
